...
import { getRoomById, joinRoom, leaveRoom, deleteRoom, getRoomParticipants, type Participant } from '../../services/room.service';
import { getRoomMessages, sendMessage, type Message } from '../../services/message.service';
  ...
  return (
    <div className="meeting-page">
      ...
      <ConfirmationModal
        ...
        message={isHost ? "Eres el anfitrión. Si sales, la reunión continuará pero sin ti. ¿Deseas salir?" : "¿Estás seguro de que deseas salir de la reunión?"}
        confirmText="Salir"
        cancelText="Cancelar"
      ...
      <div className="meeting-content">
        ...
        <div className="video-area">
            ...
            {participants.map((participant) => (
              <div key={participant.id} className="video-tile">
                <div className="participant-avatar">
                  {(participant.user?.displayName || participant.user?.nickname || participant.user?.email || 'U')[0].toUpperCase()}
                </div>
                <div className="participant-info">
                  <span className="participant-name">
                    {participant.user?.displayName || participant.user?.nickname || participant.user?.email}
          ...
          <div className="meeting-controls">
            <button className="control-btn" aria-label="Silenciar micrófono" title="Silenciar">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <button className="control-btn" aria-label="Desactivar cámara" title="Cámara">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <div className="sidebar-section chat-section">
                ...
                <div className="chat-messages">
                  {messages.map((message) => (
                    <div
                      key={message.id}
                      className={`chat-message ${message.userId === user?.id ? 'own-message' : ''}`}
                    >
                      <div className="message-avatar">
                        {(message.user?.displayName || message.user?.nickname || message.user?.email || 'U')[0].toUpperCase()}
                      </div>
                      <div className="message-content">
                        <div className="message-header">
                          <span className="message-author">
                            {message.user?.displayName || message.user?.nickname || message.user?.email}
                          </span>
                          <span className="message-time">
                            {message.createdAt ? new Date(message.createdAt).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' }) : ''}
                          </span>
                        </div>
