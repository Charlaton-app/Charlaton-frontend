...
export const login = async (credentials: LoginCredentials) => {
  try {
    ...
    console.log(
      `[AUTH-SERVICE] Authenticating with Firebase for ${credentials.email}`
    );
    ...
    console.log(
      `[AUTH-SERVICE] Firebase authentication successful: ${userCredential.user.uid}`
    );
    ...
    if (response.error) {
      console.log(
        "[AUTH-SERVICE] Backend sync failed, signing out from Firebase"
      );
    ...
    console.log(
      `[AUTH-SERVICE] Login successful for user ${response.data?.user?.id}`
    );
    return {
      data: {
...
export const signup = async (data: SignupData) => {
  ...
  try {
    ...
    if (response.error) {
      ...
      if (firebaseUser) {
        try {
          ...
          console.log(
            "[AUTH-SERVICE] Firebase user deleted due to backend error"
          );
        } catch (deleteError) {
          console.error(
            "[AUTH-SERVICE] Error deleting Firebase user:",
            deleteError
          );
        }
      }
    ...
    if (error.code === "auth/email-already-in-use") {
      ...
      return {
        error:
          "El correo ya está registrado. Intenta iniciar sesión en lugar de registrarte.",
      };
    }
...
export const loginWithGoogle = async () => {
  try {
    ...
    if (response.error) {
      console.error(
        "[AUTH-SERVICE] Backend OAuth sync failed:",
        response.error
      );
    ...
    if (error.code === "auth/popup-blocked") {
      return {
        error:
          "El navegador bloqueó la ventana emergente. Permite ventanas emergentes para continuar.",
      };
    }
  ...
  try {
    ...
    if (response.error) {
      ...
      if (
        ...
        response.error.includes("Token expirado") ||
